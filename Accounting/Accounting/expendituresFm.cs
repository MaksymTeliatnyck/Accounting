...
namespace Accounting
{
    public partial class expendituresFm : Form
    {
        ...
        public expendituresFm()
        {
            ...
            expDTPicker.Value = Convert.ToDateTime(minDate);
        ...
        private void expenditureBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Не вибрано коректну дату!", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            quantityExpSumTBox.Text = quantityExpSum.ToString("N", Utils.NumFormat(5));
        ...
        private decimal expQuantity = 0.00000m;
        private decimal expPrice = 0.00m;
        private void Calculate()
        {
            if (remainsTable.Rows.Count != 0)
            {
                decimal Quantity = Convert.ToDecimal(quantityTBox.Text.Length == 0 ? "0,00000" : quantityTBox.Text.Replace('.', ','));
                ...
                switch (expendType)
                {
                    ...
                    case ExpendTypes.ExpendByPrice:
                        ...
                        if (Price == RemainsSum)
                        {
                            ...
                            quantityTBox.Text = expQuantity.ToString("N", Utils.NumFormat(5));
        ...
        private void remainsGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (expendType == ExpendTypes.ExpendByQuantity)
                    quantityTBox.Text = remainsGrid.CurrentRow.Cells["RemainsQuantityColumn"].Value.ToString().ToString(Utils.NumFormat(5));
                else
        ...
        private void remainsGrid_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
            ...
            quantitySumTBox.Text = quantitySum.ToString("N", Utils.NumFormat(5));
        ...
        {
            Utils.EnterCheck(sender, e, 6, 5, true);
        ...
        // if credit account = 13, DateTimePicker = last work day month
        private void creditCBox_TextChanged(object sender, EventArgs e)
        {
            //DateTime now = DateTime.Now;        
            //var lastDay = new DateTime(now.Year, (now.Month-1), 1).AddMonths(1).AddDays(-1);
            //var nameDaySaturday = lastDay.DayOfWeek.Equals(DayOfWeek.Saturday);
            //var nameDaySunday = lastDay.DayOfWeek.Equals(DayOfWeek.Sunday);
            //var nameDayModay = lastDay.DayOfWeek.Equals(DayOfWeek.Monday);
            //var nameDayTuesday = lastDay.DayOfWeek.Equals(DayOfWeek.Tuesday);
            //var nameDayWednesday = lastDay.DayOfWeek.Equals(DayOfWeek.Wednesday);
            //var nameDayThursday = lastDay.DayOfWeek.Equals(DayOfWeek.Thursday);
            //var nameDayFriday = lastDay.DayOfWeek.Equals(DayOfWeek.Friday); 
            //if (creditCBox.Text.Equals("23"))
            //{
            //    if (nameDaySaturday) { expDTPicker.Value = lastDay.AddDays(-1); }
                                 
            //    if (nameDaySunday) { expDTPicker.Value = lastDay.AddDays(-2); }
                
            //    if (nameDayModay || nameDayTuesday || nameDayWednesday || nameDayThursday || nameDayFriday)
            //        expDTPicker.Value = lastDay; 
            //}   
         }
    }
}
