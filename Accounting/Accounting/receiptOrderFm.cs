...
namespace Accounting
{
    public partial class receiptOrderFm : Form
    {
        ...
        private BindingSource deliveryOrderDetailsBS = new BindingSource();
        ...
        public receiptOrderFm(bool inserting, object Type, bool isCurrencyReceipt, string Date = null, int position = -1)
        {
            ...
            if (_inserting)
            {
                ...
                //Row = DataModule.AccountingDS.Tables["DeliveryOrderDetails"].NewRow();
                //Row["OrderId"] = _id;
                ////Row["DeliveryOrderId"] = null;
                ////Row["OrderDate"] = DateTime.Now.Date;
                //DataModule.AccountingDS.Tables["DeliveryOrderDetails"].Rows.Add(Row);
            ...
            accountNumTBox.DataBindings.Add("Text", ordersBS, "Account_Num");
            ...
            #region Delivery Bindings


            deliveryOrderDetailsBS.DataSource = DataModule.AccountingDS.Tables["DeliveryOrderDetails"];
            deliveryGrid.DataSource = deliveryOrderDetailsBS;

            deliveryTtnCBox.DataBindings.Add("EditValue", deliveryOrderDetailsBS, "DeliveryOrderId");


            //deliveryTtnCBox.DataBindings.Add("Text", deliveryOrderDetailsBS, "DeliveryName");

            deliveryOrdersBS.DataSource = DataModule.ExecuteFill(@"SELECT
                                                                        do.""Id"",
                                                                        do.TTN,
                                                                        do.""Price"",
                                                                        do.""OrderDate"",
            ...
            deliveryTtnCBox.Properties.DataSource = deliveryOrdersBS;
            deliveryTtnCBox.Properties.ValueMember = "Id";
            deliveryTtnCBox.Properties.DisplayMember = "TTN";

            #endregion Delivery Bindings
        ...
        private void receiptOrderFm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            //DataModule.AccountingDS.Tables["DeliveryOrderDetails"].RejectChanges();
        }
        ...
        private void okBtn_Click(object sender, EventArgs e)
        {
            ...
            if (deliveryOrderDetailsBS.Count > 0)
            {
                if (deliveryTtnCBox.Text == "[нет данных]" || deliveryTtnCBox.Text == "[EditValue is null]")
                    messageText += "Не вказана накладна. \n";
                //if (deliveryTtnCBox.EditValue == null)
                  //  messageText += "Не указана накладная. \n";
                transportInvoiceChBox.Checked = true;
            }
            else
            {
                transportInvoiceChBox.Checked = false;
            }
            ...
            //if (supplierCBox.SelectedValue != null)
            //    supplierId = GetEmployeeId(Convert.ToInt32(((DataRowView)ordersBS.Current)["SUPPLIER_ID"]));
            //if (otkCBox.SelectedValue != null)
            //    otkId = GetEmployeeId(Convert.ToInt32(((DataRowView)ordersBS.Current)["Otk_Id"]));
            //if (storeKeeperCBox.SelectedValue != null)
            //    storekeeperId = GetEmployeeId(Convert.ToInt32(((DataRowView)ordersBS.Current)["Storekeeper_Id"]));

            if (supplierCBox.SelectedValue != null)
                supplierId = GetEmployeeId(Convert.ToInt32(supplierCBox.SelectedValue));
            if (otkCBox.SelectedValue != null)
                otkId = GetEmployeeId(Convert.ToInt32(otkCBox.SelectedValue));
            if (storeKeeperCBox.SelectedValue != null)
                storekeeperId = GetEmployeeId(Convert.ToInt32(storeKeeperCBox.SelectedValue));

            ((DataRowView)ordersBS.Current)["SupplierId"] = supplierId != null ? supplierId : (object)DBNull.Value;
            ((DataRowView)ordersBS.Current)["OtkId"] = otkId != null ? otkId : (object)DBNull.Value;
            ((DataRowView)ordersBS.Current)["StorekeeperId"] = storekeeperId != null ? storekeeperId : (object)DBNull.Value;

            //////int? supplierId = GetEmployeeId(Convert.ToInt32(((DataRowView)ordersBS.Current)["SUPPLIER_ID"]));
            //////int? otkId = GetEmployeeId(Convert.ToInt32(((DataRowView)ordersBS.Current)["Otk_Id"]));
            //////int? storekeeperId = GetEmployeeId(Convert.ToInt32(((DataRowView)ordersBS.Current)["Storekeeper_Id"]));
            ...
            DataModule.DataAdapter["DeliveryOrderDetails"].InsertCommand.Transaction = DataModule.Transaction;
            DataModule.DataAdapter["DeliveryOrderDetails"].UpdateCommand.Transaction = DataModule.Transaction;
            DataModule.DataAdapter["DeliveryOrderDetails"].DeleteCommand.Transaction = DataModule.Transaction;
            ...
            try
            {
                ...
                DataModule.DataAdapter["DeliveryOrderDetails"].Update(DataModule.AccountingDS.Tables["DeliveryOrderDetails"]);
        ...
        private void InsertMaterials()
        {
            ...
            for (int i = 0; i < DataModule.AccountingDS.Tables["Receipt"].Rows.Count; i++)
            {
                ...
                string searchMeasure = DataModule.AccountingDS.Tables["Receipt"].Rows[i]["Measure"].ToString();
                var unitLocalNameTest = (int?)DataModule.ExecuteScalar(@"SELECT FIRST 1 u.""UnitId"" FROM ""Units"" as u WHERE u.""UnitLocalName"" = '"+ searchMeasure+"'");


                //var unitLocalName = (int?)DataModule.ExecuteScalar(@"SELECT FIRST 1 u.""UnitId"" FROM ""Units"" as u WHERE u.""UnitLocalName"" = '" + DataModule.AccountingDS.Tables["Nomenclatures"].Rows[i]["Measure"].ToString().Trim() + "'");

                //if (DataModule.ExecuteScalar("SELECT FIRST 1 u.""UnitId"" FROM ""Units"" u WHERE u.""UnitLocalName"" = '" + DataModule.AccountingDS.Tables["Receipt"].Rows[i]["Measure"].ToString().Trim() + "'") !=null)
                ...
                Parameters[4].Value = unitLocalNameTest;

                DataModule.AccountingDS.Tables["Receipt"].Rows[i]["Nomenclature_Id"] = DataModule.ExecuteScalar
                    (
                        @"INSERT INTO
                            Nomenclatures(Nomenclature, Name, Measure, Balance_Account_Id, ""UnitId"")
                        VALUES
                            (@Nomenclature, @Name, @Measure, @Balance_Account_Id, @UnitId)
                        RETURNING
                            Id",
                        Parameters
                    );
        ...
        private int? GetEmployeeId(int supplierId)
        {
            FbParameter[] Parameters = 
                {
                    new FbParameter("SipplierId", supplierId)
                };

            DataModule.Connection.Open();
            var employeeId = DataModule.ExecuteScalar
                (
                    @"SELECT ""EmployeeID"" FROM ""Employees"" WHERE ""SupplierId"" = @SipplierId",
                    Parameters
                );
            DataModule.Connection.Close();

            employeeId = employeeId != DBNull.Value ? employeeId : null;
        ...
        private void nomenclNameTBox_KeyPress(object sender, KeyPressEventArgs e)
       {
        ...
        private void openTtnRBFm_Click(object sender, EventArgs e)
        {
            string dateStart = "01." + "1" + "." + DateTime.Now.Year.ToString();
            string dateEnd = "31" + "." + "12" + "." + DateTime.Now.Year.ToString();

            deliveryFm ttnFm = new deliveryFm(dateStart, dateEnd);



            if (ttnFm.ShowDialog() == DialogResult.OK)
            {
                DataRow Row = ttnFm.Return();

                updateTTNList();

                deliveryTtnCBox.EditValue = (int)Row["Id"];
                //currentId = (int)Row["Id"];
                //vendorSrnTBox.Tag = Row["Id"];
                //vendorSrnTBox.Text = Row["Srn"].ToString();
                //vendorNameTBox.Text = Row["Name"].ToString();
            ...
            //deliveryTtnCBox.EditValue = ;
            deliveryTtnCBox.Focus();
        }

        private void updateTTNList()
        {
        ...
        private void AddTTN()
        {
            //if((ttnTBox.Text.Trim().Length == 0 || deliveryCBox.Text.Trim().Length == 0 || orderDeliveryPriceTBox.Text.Trim().Length == 0) && (deliveryOrdersBS.Count > 0))
            //{
            //    MessageBox.Show("Не все поля транспортной накладной заполнены!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            //    return;
            //}

            //DataRow newRow = DataModule.AccountingDS.Tables["FixedAssetsOrder"].NewRow();

            //int deliveryNameIndex = deliveryCBox.SelectedIndex;

            //int curId = (int)deliveryTtnCBox.EditValue;

            //deliveryOrderDetailsBS.


            //DataRowView currentDeliveryDataRowView = (DataRowView)deliveryTtnCBox.Properties.GetRowByKeyValue(deliveryTtnCBox.EditValue);

            //DataRow currentDeliveryDataRow = (DataRow)currentDeliveryDataRowView.Row;


            DataRow Row = DataModule.AccountingDS.Tables["DeliveryOrderDetails"].NewRow();
            Row["OrderId"] = _id;
            //Row["Id"] = null;
            //Row["DeliveryId"] = 77;
            //Row["DeliveryOrderId"] = (int)deliveryTtnCBox.EditValue;

            //Row["TTN"] = ((DataRowView)deliveryOrdersBS.Current)["TTN"];

            DataModule.AccountingDS.Tables["DeliveryOrderDetails"].Rows.Add(Row);

            //deliveryOrderDetailsBS.Add(Row);

            deliveryOrderDetailsBS.MoveLast();
            ...
            deliveryOrderDetailsBS.EndEdit();

            deliveryTtnCBox.Enabled = true;
            ...
            //try
            //{
            //    //DataModule.DataAdapter["Order"].Update(DataModule.StorehouseDS.Tables["Order"].GetChanges());
            //    //DataModule.DataAdapter["Vat"].Update(DataModule.StorehouseDS.Tables["Order"]);
            //    //DataModule.DataAdapter["Receipt"].Update(DataModule.StorehouseDS.Tables["Receipt"]);
            //    DataModule.DataAdapter["DeliveryOrderDetails"].Update(DataModule.StorehouseDS.Tables["DeliveryOrderDetails"]);
            //    DataModule.CommitTransaction();
            //}
            ...
            //deliveryOrderDetailsBS.DataSource = DataModule.StorehouseDS.Tables["DeliveryOrderDetails"];

            //deliveryCBox.SelectedIndex = deliveryNameIndex;
        }


        private void DeleteTtn()
        {
            if (deliveryOrderDetailsBS.Count > 0)
            {
                if (MessageBox.Show("Видалити накладну?", "Подтверждение", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    deliveryOrderDetailsBS.RemoveCurrent();
            ...
            {
                MessageBox.Show("Відсутні накладні", "Подтверждение", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            }

            if (deliveryOrderDetailsBS.Count == 0)
                deliveryTtnCBox.Enabled = false;
        }
        ...
        private void deliveryTtnCBox_Leave(object sender, EventArgs e)
        {
            if (deliveryOrderDetailsBS.Count > 0)
            {
                ...
                if (deliveryTtnCBox.Text != "[нет данных]" && deliveryTtnCBox.Text != "[EditValue is null]")
                {
                    ...
                    ((DataRowView)deliveryOrderDetailsBS.Current)["TTN"] = currentDeliveryDataRow["TTN"];
                    ((DataRowView)deliveryOrderDetailsBS.Current)["Price"] = currentDeliveryDataRow["Price"];
                    ((DataRowView)deliveryOrderDetailsBS.Current)["OrderDate"] = currentDeliveryDataRow["OrderDate"];
                    ((DataRowView)deliveryOrderDetailsBS.Current)["DeliveryName"] = currentDeliveryDataRow["DeliveryName"];
                    ((DataRowView)deliveryOrderDetailsBS.Current)["DeliveryOrderId"] = currentDeliveryDataRow["Id"];
                    ((DataRowView)deliveryOrderDetailsBS.Current)["DeliveryPaymentName"] = currentDeliveryDataRow["DeliveryPaymentName"];
                    //deliveryTtnCBox.Text = currentDeliveryDataRow["TTN"].ToString();
                }
        ...
        private void deliveryTtnCBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
        ...
        private void correctionChBox_CheckedChanged(object sender, EventArgs e)
        {
