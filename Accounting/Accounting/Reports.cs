...
{class Reports
    {
        ...
        public void MSTrialBalanceByAccounts(DataTable reportTable, string StartDate, string EndDate, string FlagType)
        {
            ...
            Func<int, int, int, int> WriteContractor = (sourceId, currentPos, _contractorId) =>
            {
                ...
                cells[vsS[HeaderColumn["ContractorSrn"] - 1] + insidePos].Value = orderSource.Rows[sourceId]["ContractorSrn"]!=null ? orderSource.Rows[sourceId]["ContractorSrn"].ToString() : " ";
        ...
        public void MSTrialBalanceByAccountsCurrency(DataTable reportTable, string StartDate, string EndDate, string FlagType)
        {
            ...
            for (int i = 0; i < reportTable.Rows.Count; i++)
            {
                if (contractorId != (int)reportTable.Rows[i]["Contractor_Id"])
                {
                    ...
                    if (HeaderColumn.ContainsKey("StartDebitCurrency" + (string)reportTable.Rows[i]["CurrencyName"]))
                    ...
                    if (HeaderColumn.ContainsKey("StartCreditCurrency" + (string)reportTable.Rows[i]["CurrencyName"]))
                    ...
                    //
                    if (HeaderColumn.ContainsKey("EndDebitCurrency" + (string)reportTable.Rows[i]["CurrencyName"]))
                    ...
                    if (HeaderColumn.ContainsKey("EndCreditCurrency" + (string)reportTable.Rows[i]["CurrencyName"]))          
        ...
        public void MSTrialBalanceByAccounts39(DataTable reportTable, string StartDate, string EndDate, string FlagType)
        {
            ...
            for (int i = 0; i < reportTable.Rows.Count; i++)
            {
                if (contractorId != (int)reportTable.Rows[i]["ContractorId"])
                {
                    ...
                    if ((int?)reportTable.Rows[i]["FlagDebitCredit"] == 1)
                    {
                        cells[vsS[HeaderColumn["Payment_Document"] - 1] + currentPosition].NumberFormat = "@";
                    ...
                    {
                        cells[vsS[HeaderColumn["Order_Num"] - 1] + currentPosition].NumberFormat = "@";
                    ...
                    else if ((int?)reportTable.Rows[i]["FlagDebitCredit"] == 2)
{
        ...
        public void MSReconciliationReport(DataTable reportTable, string StartDate, string EndDate, string MaterialsServices, string ContractorName, string reportPeriodHeader)
        {
            ...
            cells["E" + currentPosition + ":" + "H" + currentPosition].Merge();
            cells["E" + currentPosition].Value = "Представник";
            currentPosition++;
            cells["A" + currentPosition + ":" + "D" + currentPosition].Merge();
            cells["A" + currentPosition].Value = "ТОВ \"НВФ \"ТЕХВАГОНМАШ\"";
            cells["E" + currentPosition + ":" + "H" + currentPosition].Merge();
            cells["E" + currentPosition + ":" + "H" + currentPosition].Value = ContractorName;
            cells["E" + currentPosition + ":" + "H" + currentPosition].WrapText = true;
            currentPosition++;
            cells["A" + currentPosition + ":" + "D" + currentPosition].Merge();
        ...
        public void ContarctorsVat(DataTable reportTable, string startDate, string endDate)
        {
            ...
            for (int i = 0; i < reportTable.Rows.Count; i++)
            {
                ...
                cells["I" + currentPosition].Value = reportTable.Rows[i]["DebitVat531"];
                cells["K" + currentPosition].Value = reportTable.Rows[i]["CreditPeriod"];
                cells["L" + currentPosition].Value = reportTable.Rows[i]["SaldoDebitEnd"];
                cells["M" + currentPosition].Value = reportTable.Rows[i]["SaldoCreditEnd"];

                currentPosition++;
                n++;
            }

            cells["A" + startPosition + ":" + ("M" + currentPosition)].Borders.LineStyle = LineStyle.Continous;
            ...
            cells["A" + currentPosition + ":" + "M" + currentPosition].Font.Size = 14;
            cells["A" + currentPosition + ":" + "M" + currentPosition].Interior.Color = Color.Bisque;

            cells["D" + currentPosition].Value = "=SUM(D" + 10 + ":" + ("D" + (currentPosition - 1)) + ")";
            ...
            */
            ...
            for (int i = 0; i < reportFixedAssetsInventoryTable.Rows.Count; i++)
            {
                ...
                //{
                    ...
                    cells[columName + currentPosition].Value = (reportFixedAssetsInventoryTable.Rows[i]["PERIODYEARAMORTIZATION"].ToString() != "0") ? reportFixedAssetsInventoryTable.Rows[i]["CURRENTAMORTIZATION"] : "0";//cells["J" + currentPosition].Value;    


                    try
                    {
                        //var fff = ColumNameAsFAM_NUM.First((x) => x.Value == reportFixedAssetsInventoryTable.Rows[i]["FAM_NUM"].ToString()).Key;
                        //string columName = ColumNameAsFAM_NUM.First((x) => x.Value == reportFixedAssetsInventoryTable.Rows[i]["FAM_NUM"].ToString()).Key;
                        //cells[columName + currentPosition].Value = (!Zero) && (reportFixedAssetsInventoryTable.Rows[i]["PERIODYEARAMORTIZATION"].ToString() != "0") ? reportFixedAssetsInventoryTable.Rows[i]["CURRENTAMORTIZATION"] : "0";//cells["J" + currentPosition].Value;
                        //cells[columName + currentPosition].Value = (reportFixedAssetsInventoryTable.Rows[i]["PERIODYEARAMORTIZATION"].ToString() != "0") ? reportFixedAssetsInventoryTable.Rows[i]["CURRENTAMORTIZATION"] : "0";//cells["J" + currentPosition].Value;    
                        var gg = (decimal)reportFixedAssetsInventoryTable.Rows[i]["CURRENTAMORTIZATION"];

                        if (gg != null)
                        {
                            if (gg > 0)
                                cells[columName + currentPosition].Value = reportFixedAssetsInventoryTable.Rows[i]["CURRENTAMORTIZATION"];
                            else
                                cells[columName + currentPosition].Value = "0.00";
                        }
        ...
        public void InvoicesForFixedAssets(DataTable SourseDataTable, string startDate, string endDate)
        {
            ...
            foreach (DataRow line in SourseDataTable.Rows)
            {
                ...
                cells[String.Format("{0}{1}", vsS[2], activRowIndex)].Value = line["GroupName"];
                cells[String.Format("{0}{1}", vsS[3], activRowIndex)].Value = line["FixedSupplierFullName"];
                cells[String.Format("{0}{1}", vsS[4], activRowIndex)].Value = line["Description"];
                cells[String.Format("{0}{1}", vsS[5], activRowIndex)].Value = line["ReceiptNum"];
                cells[String.Format("{0}{1}", vsS[6], activRowIndex)].Value = line["Nomenclature"];
                cells[String.Format("{0}{1}", vsS[7], activRowIndex)].Value = line["NomenclatureName"];
                cells[String.Format("{0}{1}", vsS[8], activRowIndex)].Value = line["BalanceAccountNum"];
                ...
                cells[String.Format("{0}{1}", vsS[11], activRowIndex)].Value = line["InvoiceSupplierFullName"];
                cells[String.Format("{0}{1}", vsS[12], activRowIndex)].Value = line["Quantity"];
                cells[String.Format("{0}{1}", vsS[13], activRowIndex)].Value = line["UnitPrice"];
                cells[String.Format("{0}{1}", vsS[13], activRowIndex)].NumberFormat = "### ### ##0.00";
                cells[String.Format("{0}{1}", vsS[14], activRowIndex)].Value = line["TotalPrice"];
                cells[String.Format("{0}{1}", vsS[14], activRowIndex)].NumberFormat = "### ### ##0.00";
                cells[String.Format("{0}{1}", vsS[15], activRowIndex)].Value = line["CreditAccountNum"];
            }
        ...
        public void FixedAssetsOrderAct(DataRowView SourseData)
        {
            ...
            if (DataModule.AccountingDS.Tables["FixedAssetsMaterials"].Rows.Count > 0)
            {
                ...
                float percentUsefullMonth = (100 / (Convert.ToInt16(SourseData["UsefulMonth"]) / 12));
        ...
        public void FixedAssetsOrderActWriteOff(DataRowView SourseData, Utils.FixedAssetsGroup group, DateTime yearAndMonth)
        {
            ...
            if (DataModule.AccountingDS.Tables["FixedAssetsMaterials"].Rows.Count > 0)
            {
                ...
                if (DataModule.AccountingDS.Tables["FixedAssetsMaterials"].AsEnumerable().Where(x => x.Field<short>("Flag") == 1 && x.Field<DateTime>("EXP_DATE").Year == yearAndMonth.Year && x.Field<DateTime>("EXP_DATE").Month == yearAndMonth.Month).Count() > 0)
                {
                    currTable = DataModule.AccountingDS.Tables["FixedAssetsMaterials"].AsEnumerable().Where(x => x.Field<short>("Flag") == 1 && x.Field<DateTime>("EXP_DATE").Year == yearAndMonth.Year && x.Field<DateTime>("EXP_DATE").Month == yearAndMonth.Month).CopyToDataTable();
                }
                else
                {
            ...
            try
            {
                worksheet.SaveAs(GeneratedReportsDir + "Акт приймання-здачі відремонтованих об'єктів інв.№" + SourseData["InventoryNumber"].ToString().Replace("/", "_") + ".xls", FileFormat.Excel8);

                Process process = new Process();
                process.StartInfo.Arguments = "\"" + GeneratedReportsDir + "Акт приймання-здачі відремонтованих об'єктів інв.№" + SourseData["InventoryNumber"].ToString().Replace("/", "_") + ".xls" + "\"";
                process.StartInfo.FileName = "Excel.exe";
                process.Start();
            }
            catch (System.IO.IOException) { MessageBox.Show("Документ уже открыт!", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
        ...
        public void FixedAssetsDecreeInput(DataRowView SourseData)
        {
            DataTable currTable = null;
            var pad = new Ua();
            SpreadsheetGear.IWorkbook workbook = SpreadsheetGear.Factory.GetWorkbook(TemplatesDir + "FixedAssetsDecreeInputTemplate.xls");
            SpreadsheetGear.IWorksheet worksheet = workbook.Worksheets[0];
            SpreadsheetGear.IRange cells = worksheet.Cells;
            ...
            int cols = 31;
            DateTime dt = DateTime.Now;
            DateTime amortizationDate = new DateTime(((DateTime)SourseData["BeginDate"]).Year, ((DateTime)SourseData["BeginDate"]).Month, 1);
            ...
            string s = ((DateTime)SourseData["BeginDate"]).Month.ToString();
            int month = Int32.Parse(s);
            string rez = RuDateAndMoneyConverter.monthNamesGenitive[month];
            cells["B" + 6].Value = ((DateTime)SourseData["BeginDate"]).Day.ToString();
            cells["D" + 6].Value = rez;
            cells["F" + 6].Value = ((DateTime)SourseData["BeginDate"]).Year.ToString();

            double years = Convert.ToDouble(SourseData["UsefulMonth"]) / 12.0;
            ...
            cells["A" + 8].Value = "« Про введення в експлуатацію " + SourseData["InventoryName"] + " Інв. " + SourseData["InventoryNumber"] + " »";
                //SourseData["InventoryName"] + ;
            cells["A" + 13].Value = " 1"+" . ";
            cells["B" + 13].Value = " Ввести в експлуатацію з " + ((DateTime)SourseData["BeginDate"]).ToShortDateString() + "р.";
            cells["B" + 14].Value =   " "+SourseData["InventoryName"];
            cells["B" + 15].Value = " Вартістю " + SourseData["BEGINPRICE"] + " грн. в кількості 1 шт.";
            cells["B" + 16].Value = " та надати наступний інвентарний номер: №" + SourseData["InventoryNumber"]+".";
            cells["A" + 18].Value = " 2 . ";
            cells["B" + 18].Value = " Для цілей бухгалтерського обліку використати рахунок №" + SourseData["BALANCE_ACCOUNT_NUM"] + ", ";
            cells["B" + 19].Value = " податкового обліку група №" + GetFixedAssetsGroupByAccountNumber(SourseData["Balance_Account_Num"].ToString())+",";
            ...
            cells["A" + 23].Value = " 3 . ";     
            cells["B" + 23].Value = " Термін корисного використання " + years + " " + RuDateAndMoneyConverter.NumberToYear(Convert.ToInt32(years)) + ".";
            cells["A" + 25].Value = " 4 . ";       
            ...
            try
            {
                worksheet.SaveAs(GeneratedReportsDir + "Наказ № 00-00-00-інв.№" + SourseData["InventoryNumber"].ToString().Replace("/", "_") + ".xls", FileFormat.Excel8);
                Process process = new Process();
                process.StartInfo.Arguments = "\"" + GeneratedReportsDir + "Наказ № 00-00-00-інв.№" + SourseData["InventoryNumber"].ToString().Replace("/", "_") + ".xls" + "\"";
                process.StartInfo.FileName = "Excel.exe";
                process.Start();
            }
            catch (System.IO.IOException) { MessageBox.Show("Документ уже открыт!", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
        ...
        public void FixedAssetsDecreeSold(DataRowView SourseData)
        {
            DataTable currTable = null;
            var pad = new Ua();
            SpreadsheetGear.IWorkbook workbook = SpreadsheetGear.Factory.GetWorkbook(TemplatesDir + "FixedAssetsDecreeSoldTemplate.xls");
            SpreadsheetGear.IWorksheet worksheet = workbook.Worksheets[0];
            SpreadsheetGear.IRange cells = worksheet.Cells;
            ...
            string s = ((DateTime)SourseData["EndRecordDate"]).Month.ToString();
            int month = Int32.Parse(s);
            string rez = RuDateAndMoneyConverter.monthNamesGenitive[month];
            cells["B" + 7].Value = ((DateTime)SourseData["EndRecordDate"]).Day.ToString();
            cells["D" + 7].Value = rez;
            cells["F" + 7].Value = ((DateTime)SourseData["EndRecordDate"]).Year.ToString();
            ...
            cells["A" + 9].Value = "« Про продаж замортизованих основних засобів " + SourseData["InventoryName"] + " інвентарний № " + SourseData["InventoryNumber"] + " »";
            cells["A" + 14].Value = " 1 . ";
            cells["B" + 14].Value = "Продати замортизований " + SourseData["InventoryName"] + ":";
            cells["B" + 15].Value = "інвентарний № " + SourseData["InventoryNumber"]+ " . ";
            cells["A" + 17].Value = " 2 . ";
            ...
            cells["B" + 23].Value = "інвентарний № " + SourseData["InventoryNumber"] + " . ";
            cells["A" + 25].Value = " 4 . ";
            ...
            try
            {
                worksheet.SaveAs(GeneratedReportsDir + "Наказ на продаж № 00-00-00-інв.№" + SourseData["InventoryNumber"].ToString().Replace("/", "_") +  ".xls", FileFormat.Excel8);

                Process process = new Process();
                process.StartInfo.Arguments = "\"" + GeneratedReportsDir + "Наказ на продаж № 00-00-00-інв.№" + SourseData["InventoryNumber"].ToString().Replace("/", "_") + ".xls" + "\"";
                process.StartInfo.FileName = "Excel.exe";
                process.Start();
            }
            catch (System.IO.IOException) { MessageBox.Show("Документ уже открыт!", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
        ...
        public void FixedAssetsDecreeAddedPrice(DataRowView SourseDataOrders, DataRowView SourseDataMaterials)
        {
            DataTable currTable = null;
            var pad = new Ua();
            SpreadsheetGear.IWorkbook workbook = SpreadsheetGear.Factory.GetWorkbook(TemplatesDir + "FixedAssetsDecreeAddedPriceTemplate.xls");
            SpreadsheetGear.IWorksheet worksheet = workbook.Worksheets[0];
            SpreadsheetGear.IRange cells = worksheet.Cells;

            //DateTime amortizationDate = new DateTime(1, Convert.ToDateTime(SourseDataMaterials["ORDER_DATE"]).AddMonths(1).Month, ((DateTime)SourseDataMaterials["ORDER_DATE"]).AddMonths(1).Year);

            DateTime amortizationDate = new DateTime(Convert.ToDateTime(SourseDataMaterials["ORDER_DATE"]).AddMonths(1).Year, Convert.ToDateTime(SourseDataMaterials["ORDER_DATE"]).AddMonths(1).Month, 1); 
            ...
            cells["D7"].HorizontalAlignment = HAlign.Center;
            cells["F7"].HorizontalAlignment = HAlign.Center;
            string s = ((DateTime)SourseDataMaterials["ORDER_DATE"]).Month.ToString();
            int month = Int32.Parse(s);
            string rez = RuDateAndMoneyConverter.monthNamesGenitive[month];
            cells["B" + 7].Value = ((DateTime)SourseDataMaterials["ORDER_DATE"]).Day.ToString();
            cells["D" + 7].Value = rez;
            cells["F" + 7].Value = ((DateTime)SourseDataMaterials["ORDER_DATE"]).Year.ToString();

            var fixedAssetsOrderName = pad.Q(SourseDataOrders["InventoryName"].ToString());

            double years = Convert.ToDouble(SourseDataOrders["UsefulMonth"]) / 12.0;
            ...
            cells["A" + 9].HorizontalAlignment = HAlign.Left;
            cells["A" + 9].Value = "« Про збільшення вартості  " + fixedAssetsOrderName[3] + "-" + " інв. №" + SourseDataOrders["InventoryNumber"] + " »";
            cells["A" + 14].Value = " 1 . ";   
            cells["B" + 14].Value = "З " + ((DateTime)SourseDataMaterials["ORDER_DATE"]).ToShortDateString() + "р. збільшити вартість " + fixedAssetsOrderName[3] + "- інв. №" + SourseDataOrders["InventoryNumber"];
            cells["B" + 16].Value = "На суму ";
            //cells["E" + 16].Value = SourseDataMaterials["UNIT_PRICE"];
            cells["E" + 16].Value = SourseDataMaterials["TOTAL_PRICE"];
            
            cells["I" + 16].Value = " грн. ";
            cells["A" + 18].Value = " 2 . ";
            cells["B" + 18].Value = "Для цілей бухгатерського обліку використовувати рахунок № " + SourseDataOrders["Balance_Account_Num"] + ", ";
            cells["B" + 19].Value = "податкового обліку група №" + GetFixedAssetsGroupByAccountNumber(SourseDataOrders["Balance_Account_Num"].ToString()) + ", ";
            ...
            try
            {
                worksheet.SaveAs(GeneratedReportsDir + "Наказ на збільшення вартості № 00-00-00-інв.№" + SourseDataOrders["InventoryNumber"].ToString().Replace("/", "_") + ".xls", FileFormat.Excel8);

                Process process = new Process();
                process.StartInfo.Arguments = "\"" + GeneratedReportsDir + "Наказ на збільшення вартості № 00-00-00-інв.№" + SourseDataOrders["InventoryNumber"].ToString().Replace("/", "_") + ".xls" + "\"";
                process.StartInfo.FileName = "Excel.exe";
                process.Start();
            }
            catch (System.IO.IOException) { MessageBox.Show("Документ уже открыт!", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
        ...
        public void FixedAssetsDecreeAddedPrice(DataRowView SourseData)
        {
            DataTable currTable = null;
            var pad = new Ua();
            SpreadsheetGear.IWorkbook workbook = SpreadsheetGear.Factory.GetWorkbook(TemplatesDir + "FixedAssetsDecreeSoldTemplate.xls");
            ...
            var fio = pad.Q(SourseData["Fio"].ToString());

            cells["A" + 9].HorizontalAlignment = HAlign.Left;
            cells["A" + 9].Value = "« Про продаж замортизованих основних засобів " + SourseData["InventoryName"] + " інвентарний № " + SourseData["InventoryNumber"] + " »";
            cells["A" + 13].Value = "        1.       Продати замортизований " + SourseData["InventoryName"] + ":";
            cells["A" + 14].Value = "інвентарний № " + SourseData["InventoryNumber"];
            cells["A" + 15].Value = "        2.       Відповідальним за продаж призначаю: " + SourseData["NameGenitive"] + " - " + fio[3];
            cells["A" + 17].Value = "        3.       Головному бухгалтеру Сергієнко Л.В. виконати необхідні бухгалтерські операції при продажу та при знятті Основнеих засобів з бухгалтерського обліку:";
            cells["A" + 19].Value = "інвентарний № " + SourseData["InventoryNumber"];
            cells["A" + 20].Value = "        4.       Контроль за виконанням цього наказу покладаю на " + SourseData["NameGenitive"] + " " + fio[3];


            try
            {
                worksheet.SaveAs(GeneratedReportsDir + "Наказ на продаж № 00-00-00-інв.№" + SourseData["InventoryNumber"].ToString().Replace("/", "_") + " " + SourseData["InventoryName"].ToString() + ".xls", FileFormat.Excel8);

                Process process = new Process();
                process.StartInfo.Arguments = "\"" + GeneratedReportsDir + "Наказ на продаж № 00-00-00-інв.№" + SourseData["InventoryNumber"].ToString().Replace("/", "_") + " " + SourseData["InventoryName"].ToString() + ".xls" + "\"";
                process.StartInfo.FileName = "Excel.exe";
                process.Start();
            }
            catch (System.IO.IOException) { MessageBox.Show("Документ уже открыт!", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
        ...
        public void FixedAssetsOrderActForSoftWare(DataRowView SourseData)
        {
            ...
            try
            {
                //worksheet.SaveAs(GeneratedReportsDir + "Акт введення в господарський оборот об'єкта права інтелектуальної властності у складі нематеріальних активів № " + SourseData["InventoryNumber"].ToString().Replace("/", "_") + ".xls", FileFormat.Excel8);
                worksheet.SaveAs(GeneratedReportsDir + "Акт введення в господарський оборот об'єкта № " + SourseData["InventoryNumber"].ToString().Replace("/", "_") + ".xls", FileFormat.Excel8);
                Process process = new Process();
                process.StartInfo.Arguments = "\"" + GeneratedReportsDir + "Акт введення в господарський оборот об'єкта № " + SourseData["InventoryNumber"].ToString().Replace("/", "_") + ".xls" + "\"";
                process.StartInfo.FileName = "Excel.exe";
                process.Start();
            }
            catch (System.IO.IOException) { MessageBox.Show("Документ уже открыт!", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
        ...
        public void FixedAssetsOrderActForSaleSoftWare(DataRowView SourseData)
        {
            SpreadsheetGear.IWorkbook workbook = SpreadsheetGear.Factory.GetWorkbook(TemplatesDir + "FixedAssetsOrderActForSaleSoftware.xls");
            SpreadsheetGear.IWorksheet worksheet = workbook.Worksheets[0];
            SpreadsheetGear.IRange cells = worksheet.Cells;
             DateTime dt = DateTime.Now;
            cells["C" + 23].Value = "1";
            cells["D" + 23].Value = SourseData["InventoryName"];
            cells["E" + 23].Value = SourseData["InventoryNumber"];
            cells["D" + 23].Value = SourseData["InventoryName"];
            cells["H" + 23].Value = SourseData["BeginDate"];
            cells["M" + 23].Value = SourseData["SUPPLIER_NAME"];
            cells["S" + 23].Value = SourseData["UsefulMonth"];
            cells["W" + 23].Value = SourseData["BALANCE_ACCOUNT_NUM"];
            cells["AL" + 23].Value = SourseData["EndRecordDate"];
            cells["AN" + 23].Value = SourseData["SoldPrice"];
            cells["AQ" + 23].Value = SourseData["TransferPrice"];
            cells["AI" + 29].Value = SourseData["SoldPrice"];
           
            try
            {
                worksheet.SaveAs(GeneratedReportsDir + "Акт вибуття (ліквідації) об'єкта права інтелектуальної власності у складі нематеріальних актів № " + SourseData["InventoryNumber"].ToString().Replace("/", "_") + ".xls", FileFormat.Excel8);

                Process process = new Process();
                process.StartInfo.Arguments = "\"" + GeneratedReportsDir + "Акт вибуття (ліквідації) об'єкта права інтелектуальної власності у складі нематеріальних актів № " + SourseData["InventoryNumber"].ToString().Replace("/", "_") + ".xls" + "\"";
                process.StartInfo.FileName = "Excel.exe";
                process.Start();
            }
            catch (System.IO.IOException) { MessageBox.Show("Документ уже открыт!", "Внимание", MessageBoxButtons.OK, MessageBoxIcon.Warning); }
        ...
        public void FixedAssetsPrintItem(string id, DataTable BaseDataTable, DateTime startDate, DateTime endDate)
        {
            /* По неизвестным причинам строка находящаяся ниже в релизной версии пк разработчика состоит из основного средства
             * которое было получено по текущему Id  в биндинге
             * в publish версии всю колекцию основных средств, 
             * в итоге для получения правильного id
             * используем id которая получаем в параметрах метода для получения нужного элемента коллекции       */
            //DataTable SourseDataTable = BaseDataTable.AsEnumerable().OrderByDescending(r => r["Id"]).CopyToDataTable();

            DataTable SourseDataTable = BaseDataTable.AsEnumerable().Where(bdsm => bdsm["Id"].ToString() == id).OrderByDescending(r => r["Id"]).CopyToDataTable();
            if (SourseDataTable.Rows.Count == 0)
            {
            ...
            //int useMonth = 0;
            //Decimal? beginPrice = 0 , monthAmortization;

            DateTime tempDate = DateTime.Now;
            int useMonth = 6;
            Decimal? beginPrice = 1000, monthAmortization = 10;
            

            //try
            //{
            //    tempDate = Convert.ToDateTime(SourseDataTable.Rows[x]["BeginDate"]);
            //    useMonth = (int)Convert.ToInt32(SourseDataTable.Rows[x]["UsefulMonth"]);
            //    beginPrice = (Decimal?)Convert.ToDecimal(SourseDataTable.Rows[x]["TOTALPRICE"]);
            //}
            //catch (Exception ex)
            //{
            //    MessageBox.Show("Ошибка преобразования данных:" + ex.Message + " " + tempDate.ToString() + " " + useMonth.ToString() + " " + beginPrice.ToString() 
        ...
        {
            public static string[] monthNamesGenitive =
            {
